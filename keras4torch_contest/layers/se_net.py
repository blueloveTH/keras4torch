...
class _SqueezeAndExcitation1d(nn.Module):
    def __init__(self, in_shape, reduction_ratio=16, channel_last=False):
        ...
        self.fc = nn.Sequential(
            k4t.layers.Lambda(lambda x: x.mean(dim=L_dim)),
            k4t.layers.Linear(C_in // reduction_ratio), nn.ReLU(),
            k4t.layers.Linear(C_in), nn.Sigmoid(),
            k4t.layers.Lambda(lambda x: x.unsqueeze(L_dim))
        )
...
class SqueezeAndExcitation1d(k4t.layers.KerasLayer):
    """
